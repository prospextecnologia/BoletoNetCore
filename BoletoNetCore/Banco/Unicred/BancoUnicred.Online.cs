...
{
    partial class BancoUnicred : IBancoOnlineRest
    {
        #region HttpClient
        ...
        private HttpClient httpClient
        {
            get
            {
                if (this._httpClient == null)
                {
                    ...
                    this._httpClient.BaseAddress = new Uri("https://cobrancaonline.Unicredi.com.br/Unicredi-cobranca-ws-ecomm-api/ecomm/v1/boleto/");
                }
        ...
        // Chave Master que deve ser gerada pelo portal do Unicredi
        ...
        public async Task<string> GerarToken()
        {
            ...
            var ret = await response.Content.ReadFromJsonAsync<ChaveTransacaoUnicrediApi>();
        ...
        {
            var emissao = new EmissaoBoletoUnicrediApi();
            ...
            // todo: verificar a necessidade de preencher dados do boleto com o retorno do Unicredi
            var boletoEmitido = await response.Content.ReadFromJsonAsync<BoletoEmitidoUnicrediApi>();
        ...
        private async Task CheckHttpResponseError(HttpResponseMessage response)
        {
            ...
            {
                var bad = await response.Content.ReadFromJsonAsync<BadRequestUnicrediApi>();
        ...
        public async Task ConsultarStatus(Boleto boleto)
        {
            ...
            var response = await this.httpClient.SendAsync(request);
            await this.CheckHttpResponseError(response);
            var ret = await response.Content.ReadFromJsonAsync<RetornoConsultaBoletoUnicrediApi[]>();
    ...
    #region Classes Auxiliares (json) Unicredi

    class InstrucaoUnicrediApi
    {
    ...
    class ChaveTransacaoUnicrediApi
    {
        public string ChaveTransacao { get; set; }
        public DateTime dataExpiracao { get; set; }
    }

    class RetornoConsultaBoletoUnicrediApi
    {
    ...
    class BoletoEmitidoUnicrediApi
    {
    ...
    class EmissaoBoletoUnicrediApi
    {
        public string Agencia { get; set; }
        public string Posto { get; set; }
        public string Cedente { get; set; }
